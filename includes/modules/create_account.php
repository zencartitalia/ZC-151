...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if (ACCOUNT_VAT == 'true') {
    if ((int)ENTRY_VAT_MIN_LENGTH > 0 && strlen($vat) < ENTRY_VAT_MIN_LENGTH) {
      $error = true;
  ...
  if (ACCOUNT_CF == 'true') {
    if ((int)ENTRY_CF_MIN_LENGTH > 0 && strlen($fiscalcode) < ENTRY_CF_MIN_LENGTH) {
      $error = true;
  ...
  // P.IVA + CF - start
    if (ACCOUNT_VAT == 'true') $sql_data_array['entry_vat'] = $vat;
    if (ACCOUNT_CF == 'true') $sql_data_array['entry_cf'] = $fiscalcode;
  // P.IVA + CF - end
