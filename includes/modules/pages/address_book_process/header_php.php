 ...
 * @version $Id: header_php.php 18695 2012-01-21 11:25:52 Spike00 aka Paolo De Dionigi $
 */
...
// P.IVA + CF - start
  ...
  if (ACCOUNT_VAT == 'true') {
    if (strlen($vat) < ENTRY_VAT_MIN_LENGTH) {
      $error = true;
      $messageStack->add('addressbook', ENTRY_VAT_ERROR);
    }
  }
  if (ACCOUNT_CF == 'true') {
    if (strlen($fiscalcode) < ENTRY_CF_MIN_LENGTH) {
      $error = true;
  ...
  // P.IVA + CF - start
    if (ACCOUNT_VAT == 'true') $sql_data_array[] = array('fieldName'=>'entry_vat', 'value'=>$vat, 'type'=>'string');
    if (ACCOUNT_CF == 'true') $sql_data_array[] = array('fieldName'=>'entry_cf', 'value'=>$fiscalcode, 'type'=>'string');
  // P.IVA + CF - end
...
*/
  $entry_query = "SELECT entry_gender, entry_company, entry_vat, entry_cf, entry_firstname, entry_lastname,
                         entry_street_address, entry_suburb, entry_postcode, entry_city,
                         entry_state, entry_zone_id, entry_country_id
                  FROM   " . TABLE_ADDRESS_BOOK . "
                  WHERE  customers_id = :customersID
